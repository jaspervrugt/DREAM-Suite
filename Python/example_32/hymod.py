...
def load_data():
    ...
    data = scipy.io.loadmat('daily_data_maurer_1980_2008.mat')

    # Extract the daily data
    daily_data = data['daily_data']

    # Create a datetime column from the data
    t_daily = pd.to_datetime(daily_data[:, 2].astype(int).astype(str) + '-' + 
        daily_data[:, 1].astype(int).astype(str) + '-' + 
        daily_data[:, 0].astype(int).astype(str))

    # Assuming daily_data has columns as described
    #t_daily = pd.to_datetime(daily_data[:, [2, 1, 0]].astype(int))  # Convert to datetime from year, month, day
    P_daily = daily_data[:, 3]                                      # Precipitation in mm/d
    Ep_daily = daily_data[:, 4]                                     # Potential evaporation in mm/d
    Q_daily = daily_data[:, 5]                                      # Discharge in mm/d

    # Calibration period
    t1 = datetime(1999, 10, 1)
    t2 = datetime(2004, 9, 30)

    # Find index for calibration period
    idx_t1 = np.where(t_daily == t1)[0][0]
    idx_t2 = np.where(t_daily == t2)[0][0]

    # Meas_info
    Meas_info = {'Y': Q_daily[idx_t1:idx_t2 + 1],
                 'sigma2': 'nonconstant'}                        # Non-constant measurement error variance

    # Define plugin structure for the model
    plugin = {}
    plugin['data'] = {}
    plugin['data']['wmp'] = 364                                     # 1 year in days
    
    idx_p = np.arange(idx_t1 - plugin['data']['wmp'], idx_t2 + 1)
    # The first two years are warm-up
    plugin = {'idx': np.arange(plugin['data']['wmp'], len(idx_p) + 1),        
              'data': {'P': P_daily[idx_p],           # Precipitation (mm/d)
                       'Ep': Ep_daily[idx_p]},         # Potential evapotranspiration (mm/d)
