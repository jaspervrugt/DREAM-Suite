...
eta_min = [ 1e-4 1e-4 1e-4 ]'; eta_max = [ inf inf 1.99 ]';
% Number of parameters
d = 3;
% How many trials with LM from different starting points?
Ntrials = 20; nu = 2; %1.3; % nu = 10;
% Initialize return matrices
[opt_LM_SWIG,min95,max95,min90,max90] = deal(nan(n_soil,d,2)); [n_LM_SWIG,SSR_LM_SWIG] = deal(nan(n_soil,1,2));
LM_res_SWIG = nan(n_soil,4,2);
...
for approach = 1:2
    for soil_type = 1:n_soil
        ...
        dat = data_SWIG{soil_type}; 
        % compute derivative
        [t_meas,I_meas,dIdt,dtdI,n_data] = data_der(dat);
        % Execute Levenberg Marquardt method 
        [eta_opt,SSR_opt,K] = least_squares_haverkamp_der(t_meas,I_meas,dIdt,dtdI,approach,eta_min,eta_max,nu,Ntrials);
        % How many trials converge to same optimum?
        [nr,Km,Kstd,Kopt] = analyze_opt(eta_opt,SSR_opt,K);
        % Store
        LM_res_SWIG(soil_type,1:4,approach) = [ nr Km Kstd Kopt ];
        %% Store output of least_squares
        zz = find(SSR_opt == min(SSR_opt)); zz = zz(1);
        % Least squares values of LM method
        opt_LM_SWIG(soil_type,1:3,approach) = eta_opt(zz,1:3);
        % SSR of least squares values of LM method
        SSR_LM_SWIG(soil_type,1,approach) = SSR_opt(zz);
        % Store number of data points
        n_LM_SWIG(soil_type,1,approach) = n_data;
        % Compute uncertainty at optimum
        [std_opt,r90,r95] = det_uncertainty_der(I_meas,t_meas,eta_opt(zz,1:3),SSR_opt(zz),eta_min,eta_max,n_data,approach);
        % Now store 95% 
        min95(soil_type,1:3,approach) = r95(1:d,1)';
        max95(soil_type,1:3,approach) = r95(1:d,2)';
        min90(soil_type,1:3,approach) = r90(1:d,1)';
        max90(soil_type,1:3,approach) = r90(1:d,2)';        
    end
end
% Save optimal values
save SWIG_646_LM_der_Jac opt_LM_SWIG SSR_LM_SWIG data_SWIG n_soil Data min95 max95 min90 max90 LM_res_SWIG
